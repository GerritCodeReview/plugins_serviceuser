...
service users. By setting this option to `false` Git over HTTP and
access to the Gerrit REST API is by default not allowed for service
users. Still Gerrit administrators may approve access for certain
service users by explicitly generating a HTTP passwords for them.
...
users. By setting this option to `false` git push is by default not
allowed for service users. Still Gerrit administrators may approve git
push for certain service users by explicitly setting email addresses
for them.
